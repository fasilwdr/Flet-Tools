...
class TaskCard(ft.Card):
    def __init__(self, top_text, name, mobile, geo_location, address, on_click=None, data_id=None, **kwargs):
        super().__init__(**kwargs)
        self.top_text, self.name, self.mobile, self.geo_location, self.address, self.on_click, self.data_id = top_text, name, mobile, geo_location, address, on_click, data_id
        ...
        return ft.Container(
            ...
            content=ft.ResponsiveRow(
                ...
                controls=[
                    ...
                    ft.IconButton(ft.icons.LOCATION_ON, padding=0, col=1.5, icon_color=PRIMARY, on_click=lambda e: self.page.launch_url(self.geo_location) if self.geo_location else None),
                    ...
                    ft.Row(controls=[
                        ft.IconButton(ft.icons.CHECK, padding=0, icon_color=ft.colors.GREEN,
                                      bgcolor=ft.colors.TRANSPARENT, on_click=self.on_click, data={"accept": self.data_id}),
                        ft.IconButton(ft.icons.CANCEL, padding=0, icon_color=ft.colors.RED,
                                      bgcolor=ft.colors.TRANSPARENT, on_click=self.on_click, data={"reject": self.data_id}),
                    ], col=3)
                ]
            )
        )
...
class TaskCardDetail(ft.Card):
    def __init__(self, top_text, status, name, address, date, count, data={}, on_click=None, on_accept_reject=None,
                 **kwargs):
        super().__init__(**kwargs)
        self.top_text, self.status, self.name, self.address, self.date, self.count, self.data, self.on_click, self.on_accept_reject = top_text, status, name, address, date, count, data, on_click, on_accept_reject
        self.elevation = 2
        self.color = ft.colors.WHITE
        self.content = self.get_content()
    ...
    def get_content(self):
        content = ft.Container(
            ...
            content=ft.ResponsiveRow(
                ...
                vertical_alignment=ft.CrossAxisAlignment.CENTER,
                controls=[
                    ft.Text(self.top_text, size=10, weight=ft.FontWeight.BOLD, color=SECONDARY, col=9),
                    ft.Container(
        ...
        if self.on_accept_reject is not None:
                    ...
                    content=ft.Row(
                        ...
                        controls=[
                            ft.ElevatedButton('Accept', icon=ft.icons.CHECK, bgcolor=ft.colors.GREEN,
                                              color=ft.colors.WHITE, on_click=self.on_accept_reject, data={"accept": self.data.get('id')}),
                            ft.ElevatedButton('Reject', icon=ft.icons.CANCEL, bgcolor=ft.colors.RED,
                                              color=ft.colors.WHITE, on_click=self.on_accept_reject, data={"reject": self.data.get('id')}),
